...
function UpstairsFridge() {
  ...
  const fetchItems = async () => {
    const querySnapshot = await getDocs(collection(db, "UpstairsFridge"));
  ...
  const addItem = async () => {
    if (newItem.name && newItem.quantity) {
      await addDoc(collection(db, "UpstairsFridge"), {
        name: newItem.name,
  ...
  const updateItem = async (id, updatedQuantity) => {
    const itemDoc = doc(db, "UpstairsFridge", id);
    ...
    <div className="container mx-auto p-4">
      <div className="w-3/4 mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="bg-blue-600 text-white py-2 px-6">
          <h1 className="text-xl font-bold text-center">UpstairsFridge</h1>
        </div>
